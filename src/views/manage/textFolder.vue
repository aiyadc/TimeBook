...
<script>
import tableHeaders from "./tableConfig/textFolder.js";
...
export default {
  ...
  methods: {
    ...
    // 拉取所有文案目录
    async fetchFolders() {
      await textRequest.getFolders(this.search).then(res => {
    ...
    handleDelete(folderid) {
      this.$confirm("确定要删除吗，目录中的所有文案也会被删除噢", "提示", {
        confirmButtonText: "是的",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        textRequest.deleteFolder(folderid).then(() => {
